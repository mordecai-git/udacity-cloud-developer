...
describe('concat', () => {
  ...
  it(`should concat Welcome and Home`, () => {
    const result = concat('Welcome', ' Home');
    expect(result).to.equal('Welcome Home');
  });

  it('should throw an error if params is empty string', () => {
    expect(() => { concat("", "Hi there") }).to.throw('empty string params');
  });
});
